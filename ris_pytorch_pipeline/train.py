    ...
    def fit(self,
        ...
        for ep in range(start_ep, epochs):
            ...
            if use_structured_R and phase_name == "joint":
                ...
                aux_soft_epochs = max(0, aux_soft_epochs)
                aux_tau = float(getattr(mdl_cfg, "AUX_MATCH_SOFT_TAU", 0.25))
                if aux_soft_epochs > 0 and ep < aux_soft_epochs:
                    self.loss_fn.set_aux_match(True, tau=aux_tau)
                    if ep == start_ep:
                        print(f"[Loss Schedule] AUX_MATCH: soft matching for first {aux_soft_epochs} epochs (tau={aux_tau:.3f})", flush=True)
                else:
                    self.loss_fn.set_aux_match(False, tau=aux_tau)
